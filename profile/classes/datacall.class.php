...
//        echo $query;
    ...
    function searchPosts($categoriesArray, $typeArray, $districtArray, $search_title, $sort_type, $is_Deliver, $is_Biding, $is_Negotiable)
    {
        $searchQuery = "";
        if (!empty($categoriesArray) && $categoriesArray != null && $categoriesArray != "") {
            $searchQuery = "WHERE (";

            for ($x = 0; $x <= (count($categoriesArray) - 1); $x++) {
                if ($x == 0) {
                    $searchQuery = $searchQuery . "preferred_user_type.iduser_type = '" . $categoriesArray[$x] . "'";
                } else {
                    $searchQuery = $searchQuery . " OR preferred_user_type.iduser_type = '" . $categoriesArray[$x] . "'";
                }
            }
            $searchQuery = $searchQuery . ")";
        }
        if (!empty($typeArray) && $typeArray != "") {
            if ($searchQuery == "") {
                $searchQuery = "WHERE (";
            } else {
            ...
            for ($x = 0; $x <= (count($typeArray) - 1); $x++) {
                if ($x == 0) {
                    $searchQuery = $searchQuery . "wastage.waste_type = '" . $typeArray[$x] . "'";
                } else {
                    $searchQuery = $searchQuery . " OR wastage.waste_type = '" . $typeArray[$x] . "'";
                }
            }
            $searchQuery = $searchQuery . ")";
        }

        if (!empty($districtArray) && $districtArray != "") {
            if ($searchQuery == "") {
                $searchQuery = "WHERE (";
            } else {
                $searchQuery = $searchQuery . " AND (";
            ...
            for ($x = 0; $x <= (count($districtArray) - 1); $x++) {
                if ($x == 0) {
                    $searchQuery = $searchQuery . "city.iddistrict = '" . $districtArray[$x] . "'";
                } else {
                    $searchQuery = $searchQuery . " OR city.iddistrict = '" . $districtArray[$x] . "'";
                }
            }
            $searchQuery = $searchQuery . ")";
        }
        if (!empty($search_title) && $search_title != "") {
            if ($searchQuery == "") {
                $searchQuery = "WHERE wastage.title LIKE '%" . $search_title . "%'";
            } else {
                $searchQuery = $searchQuery . " AND wastage.title LIKE '%" . $search_title . "%'";
            }
        }
        if (!empty($is_Deliver) && $is_Deliver != "" && $is_Deliver == "true") {
            if ($searchQuery == "") {
                $searchQuery = "WHERE wastage.is_delivery='1'";
            } else {
                $searchQuery = $searchQuery . " AND wastage.is_delivery='1'";
            }
        }
        if (!empty($is_Biding) && $is_Biding != "" && $is_Biding == "true") {
            if ($searchQuery == "") {
                $searchQuery = "WHERE wastage.isbidding='1'";
            } else {
                $searchQuery = $searchQuery . " AND wastage.isbidding='1'";
            }
        }
        if (!empty($is_Negotiable) && $is_Negotiable != "" && $is_Negotiable == "true") {
            if ($searchQuery == "") {
                $searchQuery = "WHERE wastage.isnegotiable='1'";
            } else {
                $searchQuery = $searchQuery . " AND wastage.isnegotiable='1'";
            }
        }
        $sortQuery = "";
        if (!empty($sort_type) && $sort_type != "") {
            if ($sort_type == "date") {
                $sortQuery = "wastage.date DESC";
            } else if ($sort_type == "price_l_h") {
                $sortQuery = "wastage.total_price ASC";
            } else if ($sort_type == "price_h_l") {
                $sortQuery = "wastage.total_price DESC";
            }

        }

        $query = 'SELECT wastage.idwastage,wastage.image,wastage.date,wastage.title,wastage.total_price FROM wastage INNER JOIN preferred_user_type ON wastage.idwastage = preferred_user_type.idwastage INNER JOIN city ON wastage.idcity = city.idcity ' . $searchQuery . ' AND balance_qty > 0 AND wastage.status=1 GROUP BY wastage.idwastage ORDER BY ' . $sortQuery;
...
//        return $result->fetch_all(MYSQLI_ASSOC);;
    ...
    function loadUserAccountTypes()
    {
        $query = "SELECT * FROM user_account_type";
        $result = mysqli_query($GLOBALS["__mysqli_connect"], $query);
        $temp_array = array();
        while ($getValue = mysqli_fetch_array($result)) {
            $temp_array[] = $getValue;
...
//        $temp_array = $result->fetch_all(MYSQLI_ASSOC);
    ...
    function resistorUses($type,$accountType, $cmpName, $city, $cmpAddress, $cntName, $cntNumb, $usrEmail, $usrPass, $parkImageSave, $nextCompanyCode, $nextUserCode)
    {
    ...
    VALUES ('$nextCompanyCode', '$city', '$cmpName','$cmpAddress' ,'$cntName' ,'$cntNumb' , '$parkImageSave','$dateee' ,'$dateee' )";
        ...
        $query = "INSERT INTO `user` (`iduser`, `iduser_type`, `idcompany_reg`, `iduser_account_type`, `name`, `email`, `password`, `contact_no`, `created_at`, `updated_at`, `status`) 
    VALUES ('$nextUserCode', '$type', '$nextCompanyCode', '$accountType', '$cntName', '$usrEmail', '$usrDpass', '$cntNumb', '$dateee', '$dateee', '2')";
        $result = mysqli_query($GLOBALS["__mysqli_connect"], $query);
    }
